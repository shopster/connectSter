...
implements ICommand<IAdapterData>
{
    ...
    public IResponse<IAdapterData> execute( Session session, Map<String, Object> parameters )
    {
        ...
        try
        {
            ...
            if( data == null )
            {
                throw new AdapterException( "Failed to load adapter data for: " + adapterId + ", " + category + ", " + name );
            }

            response = new Response<IAdapterData>( data, IResponse.Status.Successful, "Loaded adapter data for: " +
